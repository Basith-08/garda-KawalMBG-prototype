...
const previewDistribution = computed<Distribution | null>(() => {
  ...
  return {
    ...
    suhu: draft.value.suhu ?? 32,
    durasi: draft.value.durasi ?? 0,
    levelRisiko: 'MEDIUM',
    catatan: draft.value.evidenceUploaded
      ? 'Preview operasional memakai timeline produksi, dispatch, dan bukti QC harian.'
      : 'Preview operasional memakai timeline harian dengan bukti QC yang belum lengkap.',
    cookedAt: draft.value.jamMasak,
    packagedAt: draft.value.jamPlating,
    pickupAt: draft.value.jamBerangkat,
    deliveredAt: draft.value.estimasiTiba,
    arrivalStatus: 'not_confirmed',
    qcPhotoUploaded: Boolean(draft.value.qcPhotoUploaded),
    productionPhotoUploaded: Boolean(draft.value.productionPhotoUploaded),
    packagingPhotoUploaded: Boolean(draft.value.packagingPhotoUploaded),
    vehiclePhotoUploaded: Boolean(draft.value.vehiclePhotoUploaded),
    evidenceUploaded: Boolean(draft.value.evidenceUploaded),
  }
})
...
function confirm() {
  if (draft.value) {
    ...
    draft.value.schools.forEach((school: string) => {
      const dist: Distribution = {
        ...
        catatan: assessment?.operationalSummary || 'Data dikirim via aplikasi. Menunggu konfirmasi sekolah.',
        cookedAt: draft.value.jamMasak,
        packagedAt: draft.value.jamPlating,
        pickupAt: draft.value.jamBerangkat,
        deliveredAt: draft.value.estimasiTiba,
  ...
  toast.add({ severity: 'success', summary: 'Distribusi Dimulai', detail: 'Timeline, bukti QC, dan status konfirmasi sekolah sudah direkam', life: 4000 })
